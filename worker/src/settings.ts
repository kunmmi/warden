...
export function mergeSettings(
  ...
  return {
    bundlerApiKey: str(file.bundlerApiKey, env.WARDEN_BUNDLER_API_KEY),
    bundlerUrl: str(file.bundlerUrl, env.WARDEN_BUNDLER_URL),
    rpcMainnet: str(file.rpcMainnet, env.WARDEN_RPC_MAINNET),
    rpcTestnet: str(file.rpcTestnet, env.WARDEN_RPC_TESTNET),
    groqApiKey: str(file.groqApiKey, env.GROQ_API_KEY),
    groqModel: str(file.groqModel, env.WARDEN_GROQ_MODEL, d.groqModel)!,
    anthropicApiKey: str(file.anthropicApiKey, env.ANTHROPIC_API_KEY),
    llmProvider: str(file.llmProvider, env.WARDEN_LLM_PROVIDER),
    llmApiKey: str(file.llmApiKey, env.WARDEN_LLM_API_KEY),
    llmBaseUrl: str(file.llmBaseUrl, env.WARDEN_LLM_BASE_URL),
    llmProviderModel: str(file.llmProviderModel, env.WARDEN_LLM_PROVIDER_MODEL),
    rialtoApiKey: str(file.rialtoApiKey, env.WARDEN_RIALTO_API_KEY),
    rialtoApiKeyHeader: str(file.rialtoApiKeyHeader, env.WARDEN_RIALTO_API_KEY_HEADER, d.rialtoApiKeyHeader)!,
    breakerAddress,
    paperTradingEnabled: bool(file.paperTradingEnabled, env.WARDEN_PAPER_TRADING, d.paperTradingEnabled),
    paperStartUsdg: num(file.paperStartUsdg, env.WARDEN_PAPER_START_USDG, d.paperStartUsdg, 1, 10_000_000),
    ...
    basketSymbols,
    customTokens,
    minPoolLiquidityUsdg: num(file.minPoolLiquidityUsdg, env.WARDEN_MIN_POOL_LIQUIDITY_USDG, d.minPoolLiquidityUsdg, 0, 100_000_000),
    maxPriceDivergenceBps: num(file.maxPriceDivergenceBps, env.WARDEN_MAX_PRICE_DIVERGENCE_BPS, d.maxPriceDivergenceBps, 10, 10_000),
    discoveryEnabled: bool(file.discoveryEnabled, env.WARDEN_DISCOVERY_ENABLED, d.discoveryEnabled),
    discoveryIntervalMin: num(file.discoveryIntervalMin, env.WARDEN_DISCOVERY_INTERVAL_MIN, d.discoveryIntervalMin, 1, 1440),
    scoutEnabled: bool(file.scoutEnabled, env.WARDEN_SCOUT_ENABLED, d.scoutEnabled),
    scoutBudgetUsdg: num(file.scoutBudgetUsdg, env.WARDEN_SCOUT_BUDGET_USDG, d.scoutBudgetUsdg, 0, 1_000_000),
    scoutPerTokenUsdg: num(file.scoutPerTokenUsdg, env.WARDEN_SCOUT_PER_TOKEN_USDG, d.scoutPerTokenUsdg, 0, 1_000_000),
    buyPerTickUsdg: num(file.buyPerTickUsdg, env.WARDEN_BUY_PER_TICK_USDG, d.buyPerTickUsdg, 1, 100_000),
    idleFloorUsdg: num(file.idleFloorUsdg, env.WARDEN_IDLE_FLOOR_USDG, d.idleFloorUsdg, 0, 1_000_000),
    gapEnterBudgetUsdg: num(file.gapEnterBudgetUsdg, env.WARDEN_GAP_BUDGET_USDG, d.gapEnterBudgetUsdg, 1, 1_000_000),
    ...
    telegramControlEnabled: bool(file.telegramControlEnabled, env.WARDEN_TELEGRAM_CONTROL, d.telegramControlEnabled),
    telegramAllowlist: numArray(file.telegramAllowlist, env.WARDEN_TELEGRAM_ALLOWLIST, d.telegramAllowlist),
    telegramMaxActionUsdg: num(file.telegramMaxActionUsdg, env.WARDEN_TELEGRAM_MAX_ACTION_USDG, d.telegramMaxActionUsdg, 1, 100_000),
    telegramTransferEnabled: bool(file.telegramTransferEnabled, env.WARDEN_TELEGRAM_TRANSFER, d.telegramTransferEnabled),
    telegramTransferDailyUsdg: num(file.telegramTransferDailyUsdg, env.WARDEN_TELEGRAM_TRANSFER_DAILY_USDG, d.telegramTransferDailyUsdg, 1, 1_000_000),
    telegramNotifyEnabled: bool(file.telegramNotifyEnabled, env.WARDEN_TELEGRAM_NOTIFY, d.telegramNotifyEnabled),
    telegramNotifyEveryMin: num(file.telegramNotifyEveryMin, env.WARDEN_TELEGRAM_NOTIFY_EVERY_MIN, d.telegramNotifyEveryMin, 0, 1440),
    telegramDigestHour: num(file.telegramDigestHour, env.WARDEN_TELEGRAM_DIGEST_HOUR, d.telegramDigestHour, 0, 23),
    telegramPcControlEnabled: bool(file.telegramPcControlEnabled, env.WARDEN_TELEGRAM_PC_CONTROL, d.telegramPcControlEnabled),
    telegramCapabilities: strArray(file.telegramCapabilities, env.WARDEN_TELEGRAM_CAPABILITIES, d.telegramCapabilities),
    telegramFilesRoot: str(file.telegramFilesRoot, env.WARDEN_TELEGRAM_FILES_ROOT),
    telegramShellAllowlist: strArray(file.telegramShellAllowlist, env.WARDEN_TELEGRAM_SHELL_ALLOWLIST, d.telegramShellAllowlist),
    telegramAppAllowlist: strArray(file.telegramAppAllowlist, env.WARDEN_TELEGRAM_APP_ALLOWLIST, d.telegramAppAllowlist),
    telegramTranscribeKey: str(file.telegramTranscribeKey, env.WARDEN_TELEGRAM_TRANSCRIBE_KEY),
    telegramTranscribeBase: str(file.telegramTranscribeBase, env.WARDEN_TELEGRAM_TRANSCRIBE_BASE, d.telegramTranscribeBase)!,
    telegramAgentEnabled: bool(file.telegramAgentEnabled, env.WARDEN_TELEGRAM_AGENT, d.telegramAgentEnabled),
    telegramAgentAutoShell: bool(file.telegramAgentAutoShell, env.WARDEN_TELEGRAM_AGENT_AUTOSHELL, d.telegramAgentAutoShell),
    telegramAgentMaxSteps: num(file.telegramAgentMaxSteps, env.WARDEN_TELEGRAM_AGENT_MAX_STEPS, d.telegramAgentMaxSteps, 1, 60),
  };
}
 ...
 * or a query param. If the URL names a BSC chain id that ISN'T the grant's,
...
export function bundlerChainMismatch(bundlerUrl: string | undefined, grantChainId: number): number | null {
  ...
  const ids = [...bundlerUrl.matchAll(/(?:\/|=)(56|97)(?:\/|$|&|\?)/g)].map((m) => Number(m[1]));
